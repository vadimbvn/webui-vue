...
export default {
  name: 'VirtualMedia',
  components: { PageSection, ModalConfigureConnection, FormFile },
  ...
  methods: {
    startVM(device) {
      ...
      device.nbd.socketStarted = () =>
        this.successToast(this.$t('pageKvm.toast.serverRunning'));
      device.nbd.errorReadingFile = () =>
        this.errorToast(this.$t('pageKvm.toast.errorReadingFile'));
      device.nbd.socketClosed = (code) => {
        if (code === 1000)
          this.successToast(this.$t('pageKvm.toast.serverClosedSuccessfully'));
        else this.errorToast(this.$t('pageKvm.toast.serverClosedWithErrors'));
        device.file = null;
    ...
    startLegacy(connectionData) {
      ...
      this.$store
        ...
        .then(() => {
          this.successToast(
            this.$t('pageKvm.toast.serverConnectionEstablished')
          );
          connectionData.isActive = true;
        })
        .catch(() => {
          this.errorToast(this.$t('pageKvm.toast.errorMounting'));
          this.isActive = false;
        })
        .finally(() => this.endLoader());
    },
    stopLegacy(connectionData) {
      this.$store
        ...
        .catch(() => this.errorToast(this.$t('pageKvm.toast.errorUnmounting')))
        .finally(() => this.endLoader());
    },
